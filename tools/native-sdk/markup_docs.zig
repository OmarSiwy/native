...
pub const element_docs = [_]Doc{
    ...
    .{ .name = "dropdown-menu", .doc = "Vertical menu surface; children are menu-item elements. anchor=\"below|above|right|left\" floats it against its parent (put it beside its trigger in a stack): late z-pass above the whole tree, window-clipped, auto-flipping at the edges. Pair with on-dismiss so Escape/click-outside close model-side." },
...
pub const anchor_attr_docs = [_]Doc{
    .{ .name = "anchor", .doc = "dropdown-menu and tooltip: floats the surface against its PARENT's frame instead of the flow (literal below, above, right, or left; either side auto-flips at the window edges). Late z-pass above the whole tree, window-clipped — never cropped by a scroll pane, never reflows siblings. Put the surface beside its trigger inside a stack. An anchored tooltip's visibility is runtime-owned (hover intent on the trigger; see tooltip-delay), unlike the model-owned dropdown." },
...
pub const event_docs = [_]Doc{
    ...
    .{ .name = "on-hover", .doc = "Dispatch a Msg on hover-enter: tag or tag:{payload}. Fires once per resolved target change (the pointer entering the element), not per pointer sample. Legal on any element - binding it makes the element a hover hit target." },
